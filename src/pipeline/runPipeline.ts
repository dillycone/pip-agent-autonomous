...
export async function runPipeline(params: RunPipelineParams): Promise<void> {
  ...
  try {
    try {
      ...
      logger.error("Worker process error", workerError);
      logger.warn("Checking for output file...");

      const recoveryResult = handleWorkerErrorRecovery(
        outputPath,
        projectRoot,
        pipModelFamily,
        emit,
      ...
      if (recoveryResult.recovered) {
        emit("final", {
          ok: true,
          docx: sanitizePath(recoveryResult.finalDocxPath!),
          docxRelative: recoveryResult.docxRelative,
          recovered: true
        });
        hasFinalEvent = true;
        setRunStatus("success");
    ...
    if (!hasFinalEvent && ["running", "pending"].includes(currentRunStatus)) {
      logger.warn(`No final event emitted, status: ${currentRunStatus}`);
    ...
    logger.info(`Pipeline completed - status: ${currentRunStatus}, hasFinalEvent: ${hasFinalEvent}`);
    clearPreviewTimeouts();
  ...
  logger.debug("runCore function defined successfully");
  logger.debug(`ENABLE_DRAFT_STREAMING: ${ENABLE_DRAFT_STREAMING}`);

  if (ENABLE_DRAFT_STREAMING) {
    logger.info("Running with draft stream context");
    ...
    logger.info("Running without draft stream context");
    await runCore();
  }

  logger.info(`Pipeline completed for ${runId}`);
}
