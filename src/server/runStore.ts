...
class RunStore {
  ...
  subscribe(
    ...
    if (record.abortTimeout) {
      clearTimeout(record.abortTimeout);
      record.abortTimeout = null;
    }
    ...
    const unsubscribe = () => {
      ...
      if (record.subscribers === 0 && (record.status === "pending" || record.status === "running")) {
        record.abortTimeout = setTimeout(() => {
          record.abortTimeout = null;
          if (record.subscribers === 0 && (record.status === "pending" || record.status === "running")) {
            this.abort(runId, "Client disconnected");
          }
        }, DISCONNECT_GRACE_MS);
        record.abortTimeout.unref?.();
      }
    };
  ...
  finish(runId: string): void {
    ...
    record.emitter.removeAllListeners("event");
    if (!record.controller.signal.aborted) {
      record.controller.abort(new Error("Run finished"));
    }
    record.updatedAt = Date.now();
    setTimeout(() => {
  ...
  private cleanup(): void {
    ...
    for (const [runId, record] of this.runs.entries()) {
      if (now - record.updatedAt > RUN_TTL_MS) {
        record.emitter.removeAllListeners("event");
        if (!record.controller.signal.aborted) {
          record.controller.abort(new Error("Run expired"));
        }
        this.runs.delete(runId);
      }
    }
